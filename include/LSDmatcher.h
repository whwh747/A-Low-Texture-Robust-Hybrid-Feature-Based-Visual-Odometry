...
namespace ORB_SLAM2
{
    ...
    public:
        ...
        int SearchByGeomNApearance(Frame &CurrentFrame, const Frame &LastFrame, const float desc_th,vector<int> &matches_12);

        /// 使用RANSAC拟合直线
        static std::pair<double, double> fitLineRANSAC(const std::vector<cv::Point2f>& points, int numIterations, double distanceThreshold);

        int optical_flow_line(Frame &CurrentFrame, const Frame &LastFrame,vector<int> &matchs_12,cv::Mat Rcl);

        ///线段我们改用描述子匹配
        int SearchByDescriptor(KeyFrame *pKF, Frame &currentF, vector<MapLine *> &vpMapLineMatches);
