...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn), vManhAxisIdx(F.vManhAxisIdx),
    mvPerpLines(F.mvPerpLines), mvParallelLines(F.mvParallelLines),
    ...
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap), mvLines3D(F.mvLines3D),
    NL(F.NL), mvKeyLines(F.mvKeylinesUn), mvLineEq(F.mvLineEq), mvKeyLineFunctions(F.mvKeyLineFunctions), mLineDescriptors(F.mLdesc),
    mvpMapLines(F.mvpMapLines), ImageGray(F.ImageGray.clone()),
    mvPlanePoints(F.mvPlanePoints),mvPlaneCoefficients(F.mvPlaneCoefficients),mvpMapPlanes(F.mvpMapPlanes),
    mvpParallelPlanes(F.mvpParallelPlanes),mvpVerticalPlanes(F.mvpVerticalPlanes),
    mnPlaneNum(F.mnPlaneNum),mbNewPlane(F.mbNewPlane),fa(F.fa),pOnL(F.pOnL)
{
    mnId=nNextId++;
...
void KeyFrame::UpdateConnections()
{
    ...
    //For all map points/map lines in keyframe check in which other keyframes are they seen
...
float KeyFrame::ComputeSceneMedianDepth(const int q)
{
    ...
    Vector6d KeyFrame::obtain3DLine(const int &i) {
        Vector3d st_pt = mvLines3D[i].first;
        Vector3d end_pt = mvLines3D[i].second;

        cv::Mat Ac = (Mat_<float>(3, 1) << st_pt(0), st_pt(1), st_pt(2));
        cv::Mat A = Twc.rowRange(0,3).colRange(0,3) * Ac + Twc.rowRange(0,3).col(3);
        cv::Mat Bc = (Mat_<float>(3, 1) << end_pt(0), end_pt(1), end_pt(2));
