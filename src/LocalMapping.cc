    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                //cout<<"mpTracker->match_true = "<<mpTracker->match_true<<endl;

                // VI-D Local BA
                //if(mpMap->KeyFramesInMap()>3 && mpTracker->match_true > 180)
                if(mpMap->KeyFramesInMap()>3 && mpTracker->match_true > 250)
                {
                    //if(mpTracker->match_true < 100)continue;
...
//        auto dTh = 0.05;
//        auto aTh = 0.05;
        auto dTh = ORB_SLAM2::Config::Get<double>("Plane.AssociationDisRef");
